...
//-----------------------------------------------------------------------------------------------
	endTime = clock();
	cout<<"Time Cost[Rout solve time]-------------->"<<(double)(endTime - startTime)/CLOCKS_PER_SEC<<endl;
	startTime=endTime;
	...
	cout<<routeSolution[IndexLowestCost]->getCost()<<endl;
...
size_t Route::costCal(NetworkInfo networkInfo)
{
	...
	cout<<"num of server: "<<serverUsed.size()<<endl;
	return cost;
}
